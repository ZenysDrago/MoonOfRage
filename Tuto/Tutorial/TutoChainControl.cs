...
public class TutoChainControl : MonoBehaviour
{
    [SerializeField] GameObject sphere;
    [SerializeField] GameObject player;
    [SerializeField] float maxLink = 10f;
    private TutoAnimator anim;
    ...
    private void Start()
    {
        StopAllCoroutines();
        triggerInputTimer = 0;
        anim = GetComponent<TutoAnimator>();
    ...
    {
        if (player.GetComponent<Player>().state == PlayerState.COMBO_MOON || !manager.moonOn)
            return;
        ...
        if (context.canceled)
        {
            if (triggerInputTimer <= timeBetweenTractAndGrabSphere && manager.recallOn)
            {
                sphere.GetComponent<Rigidbody>().constraints = RigidbodyConstraints.None;
